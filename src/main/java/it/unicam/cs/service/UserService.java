...
public class UserService {
    ...
    public UserDTO createProducer(String name) {
        ...
        UserEntity saved = producerRepo.save((Producer)user);
        return mapper.toDTO(saved);
    }

    public UserDTO createTransformer(UserDTO dto) {
        User user = transformerFactory.createUser(dto.getName());
        UserEntity saved = transformerRepo.save((Transformer)user);
        return mapper.toDTO(saved);
    }

    public UserDTO createDistributor(UserDTO dto) {
        User user = distributorFactory.createUser(dto.getName());
        UserEntity saved = distributorRepo.save((Distributor) user);
        return mapper.toDTO(saved);
    }
